...
修正后的 Project Zomboid 样本 14 已完成第一次复测：连续 41 次无新帧期间，探针
保持每 2 秒轮询，单次抓帧最大 38.172ms，确认应用层无限等待已修复；但 130 次轮询
中仍有 49 次没有 WGC 新帧，且产品负责人再次看到游戏内光标消失。因此鼠标症状
不是阻塞式 `grab()` 的必要结果，下一步必须用光标合成开关做单变量 A/B。该样本
仍属诊断数据，不进入正式玩法指标样本。
...
其中两个 Project Zomboid 会话因取帧端缺陷不计入正式数据，仅作诊断证据。另补测
2.0 / 1.0 秒轮询的 CPU 与内存；上下文、指标与逐帧人工复核集中记录在

# M5-B-T3a OpenAI GPT-5.4 Mini 游戏知识探针报告

本轮只验证修改后的详细游戏 context 提示词，不替换上一轮正式 3 模型 × 15 游戏判卷，也不作选型推荐。

## 模型与样本

- 模型：[OpenAI GPT-5.4 Mini](https://openrouter.ai/openai/gpt-5.4-mini)（`openai/gpt-5.4-mini`）。
...
|---|---:|---:|---:|---:|---:|---:|
| 联网模式 | 0/3 | 0/3 | 0/3 | 0.033 / 0.070 / 0.079 | 0/3 | $0.000000000 |

## 逐次结果

| 游戏 | 模式 | 状态 | 实际模型／上游 | 耗时（秒） | 输入／输出／推理 token | finish_reason | 花费（USD） |
|---|---|---|---|---:|---:|---|---:|
| Marvel Rivals | online | 失败 | — / — | 0.079 | —/—/— | — | — |
| EA Sports FC 26 | online | 失败 | — / — | 0.022 | —/—/— | — | — |
| Hades II | online | 失败 | — / — | 0.033 | —/—/— | — | — |
...
- `marvel-rivals` / `online`：OpenRouter请求失败（HTTP 404）：No endpoints found that can handle the requested parameters. To learn more about provider routing, visit: https://openrouter.ai/docs/guides/routing/provider-selection [status=404 provider_code=404 request_id=gen-1788236042-0Tb3pPJOFur665BxovgK request_id_header=x-generation-id occurred_at=2026-09-01T04:14:02.103356+00:00 profile_name=m5-b-t3a-v2:online]
- `ea-sports-fc-26` / `online`：OpenRouter请求失败（HTTP 404）：No endpoints found that can handle the requested parameters. To learn more about provider routing, visit: https://openrouter.ai/docs/guides/routing/provider-selection [status=404 provider_code=404 request_id=gen-1788236042-yLDNBILV9vwNPBzA6HTe request_id_header=x-generation-id occurred_at=2026-09-01T04:14:02.126156+00:00 profile_name=m5-b-t3a-v2:online]
- `hades-ii` / `online`：OpenRouter请求失败（HTTP 404）：No endpoints found that can handle the requested parameters. To learn more about provider routing, visit: https://openrouter.ai/docs/guides/routing/provider-selection [status=404 provider_code=404 request_id=gen-1788236042-dHu6JXl9MOLs0udUn2Gq request_id_header=x-generation-id occurred_at=2026-09-01T04:14:02.160395+00:00 profile_name=m5-b-t3a-v2:online]
...
- 开始：`2026-09-01T04:14:01.991144+00:00`
- 结束：`2026-09-01T04:14:02.161394+00:00`

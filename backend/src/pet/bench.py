"""Offline A/B evaluation of template and model-generated CS2 commentary."""

from __future__ import annotations

import argparse
from collections.abc import Sequence
from dataclasses import dataclass
from datetime import datetime, timezone
import math
from pathlib import Path
import random
import re
import statistics
import sys
from typing import Any, Literal

from pet.commentary import commentary_category
from pet.commentary_rules import CALLOUT_TERMS, FORBIDDEN_RAW_CURSES
from pet.commentary_templates import (
    COMMENTARY_TEMPLATES,
    WIN_METHOD_LABELS,
    CommentaryCategory,
)
from pet.config import PersonalityStyle, load_config
from pet.events import EventType, GameEvent
from pet.gsi import GameSnapshot
from pet.llm import (
    LlmClientProtocol,
    LlmError,
    LlmResult,
    OpenRouterClient,
)
from pet.replay import load_recording, replay_commentary

PromptVariant = Literal["bare", "styled"]

# Short emotional lines benefit from variety; this benchmark measures taste, not determinism.
TEMPERATURE = 0.9
MAX_COMPLETION_TOKENS = 96
BLINDING_SEED = 20260810

_REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
_COMMUNITY_LORE_PATH = _REPOSITORY_ROOT / "docs" / "中文CS社群常见梗和语录.md"
_PLACEHOLDER_PATTERN = re.compile(r"\{[^{}]+\}")
_HAN_CHARACTER_PATTERN = re.compile(r"[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff]")

_EVENT_LABELS: dict[EventType, str] = {
    "kill": "普通击杀",
    "kill_headshot": "爆头击杀",
    "multi_kill": "多杀",
    "death": "死亡",
    "death_after_kill": "击杀后被补枪",
    "death_thrown_away": "死亡（白给）",
    "round_win": "回合胜利",
    "round_loss": "回合失败",
}
_CATEGORY_LABELS: dict[CommentaryCategory, str] = {
    "kill": "普通击杀",
    "kill_headshot": "爆头击杀",
    "multi_2": "双杀",
    "multi_3": "三杀",
    "multi_4": "四杀",
    "multi_5": "五杀",
    "multi_general": "多杀",
    "death": "死亡",
    "death_after_kill": "击杀后被补枪",
    "death_thrown_away": "白给",
    "round_win_elimination": "回合胜利·灭队",
    "round_win_bomb": "回合胜利·引爆",
    "round_win_defuse": "回合胜利·拆包",
    "round_win_time": "回合胜利·时间到",
    "round_win_general": "回合胜利·通用",
    "round_loss_elimination": "回合失败·灭队",
    "round_loss_bomb": "回合失败·引爆",
    "round_loss_defuse": "回合失败·拆包",
    "round_loss_time": "回合失败·时间到",
    "round_loss_general": "回合失败·通用",
}
_FACT_LABELS: dict[str, str] = {
    "round_kill_index": "本回合第几杀",
    "delta": "本次击杀增量",
    "weapon": "武器",
    "count": "本回合累计击杀",
    "survival_seconds": "存活秒数",
    "round_kills": "本回合击杀数",
    "seconds_since_last_kill": "距上次击杀秒数",
    "equip_value": "装备价值",
    "score_situation": "比分态势",
    "team_consecutive_round_losses": "本方连败轮数",
    "method": "回合获胜方式",
    "score_ct": "CT 比分",
    "score_t": "T 比分",
}


@dataclass(frozen=True, slots=True)
class LengthStatistics:
    """Chinese-character distribution of one personality's template pool."""

    minimum: int
    median: float
    p90: int
    maximum: int


@dataclass(frozen=True, slots=True)
class OutputChecks:
    """Factual violations observed in one successful model output."""

    exceeds_max_chars: bool
    callout_terms: tuple[str, ...]
    raw_curses: tuple[str, ...]


@dataclass(frozen=True, slots=True)
class BenchAttempt:
    """One prompt variant's success or failure for one selected event."""

    variant: PromptVariant
    result: LlmResult | None
    error: str | None
    checks: OutputChecks | None


@dataclass(frozen=True, slots=True)
class BenchEvent:
    """One selected production event and both model attempts."""

    event: GameEvent
    category: CommentaryCategory
    template_text: str
    map_name: str | None
    score_ct: int | None
    score_t: int | None
    attempts: tuple[BenchAttempt, BenchAttempt]


@dataclass(frozen=True, slots=True)
class BenchResult:
    """All data needed to render one self-contained Markdown report."""

    recording_path: Path
    requested_model: str
    personality_style: PersonalityStyle
    run_timestamp: datetime
    snapshot_count: int
    detected_event_count: int
    selected_event_count: int
    truncated_event_count: int
    length_statistics: LengthStatistics
    events: tuple[BenchEvent, ...]
    blinding_seed: int = BLINDING_SEED


def calculate_length_statistics(texts: Sequence[str]) -> LengthStatistics:
    """Calculate nearest-rank P90 after removing format placeholders."""
    lengths = sorted(_chinese_character_count(text) for text in texts)
    if not lengths:
        raise ValueError("cannot calculate template lengths from an empty sequence")
    p90_index = math.ceil(len(lengths) * 0.9) - 1
    return LengthStatistics(
        minimum=lengths[0],
        median=float(statistics.median(lengths)),
        p90=lengths[p90_index],
        maximum=lengths[-1],
    )


def commentary_length_statistics(
    personality_style: PersonalityStyle,
) -> LengthStatistics:
    """Measure all template lines for the selected personality."""
    texts = tuple(
        template.text
        for templates in COMMENTARY_TEMPLATES[personality_style].values()
        for template in templates
    )
    return calculate_length_statistics(texts)


def build_system_prompt(
    variant: PromptVariant,
    *,
    personality_style: PersonalityStyle,
    category: CommentaryCategory,
    max_chars: int,
    community_lore: str | None = None,
) -> str:
    """Build one blinded experiment prompt without changing production templates."""
    role = (
        "你是陪朋友打 CS2 的中文游戏搭子，反应自然、简短，偶尔损一句但不刻薄。"
        if personality_style == "brother"
        else "你是看朋友打 CS2 的中文赛事解说，反应利落、有现场感但不端着。"
    )
    shared = "\n".join(
        (
            role,
            "只输出一句话本身，不要引号、不要解释、不要前后缀。",
            f"这句话最多包含 {max_chars} 个汉字。",
            "不得出现任何地图点位称呼，例如 A点、B洞、中路、狗洞、跳台、包点。GSI 不提供玩家位置，不能猜。",
            "回合胜负类事件不得暗示是玩家本人执行的；GSI 不提供获胜的具体执行者。",
            "不强制使用人称，中文口语可以省略主语。",
        )
    )
    if variant == "bare":
        return shared
    if variant != "styled":
        raise ValueError(f"unknown prompt variant: {variant}")

    lore = community_lore
    if lore is None:
        lore = _COMMUNITY_LORE_PATH.read_text(encoding="utf-8")
    examples = "\n".join(
        f"- {_remove_placeholders(template.text)}"
        for template in COMMENTARY_TEMPLATES[personality_style][category]
    )
    return (
        f"{shared}\n\n"
        "下面是同类事件的全部手写风格样例。只模仿语气，不要解释样例：\n"
        f"{examples}\n\n"
        "下面是中文 CS 社群背景梗库。它只提供语言背景，不代表每条都适合本次事件：\n"
        f"{lore}"
    )


def build_user_prompt(
    event: GameEvent,
    *,
    category: CommentaryCategory,
    map_name: str | None,
    score_ct: int | None,
    score_t: int | None,
) -> str:
    """Describe one event's structured facts in natural Chinese rather than JSON."""
    facts = "；".join(
        f"{_FACT_LABELS.get(key, key.replace('_', ' '))}：{_display_fact(key, value)}"
        for key, value in event.facts.items()
    )
    if not facts:
        facts = "没有更多可用事实"
    map_label = map_name or "未知"
    score_label = (
        f"CT {score_ct} 比 {score_t} T"
        if score_ct is not None and score_t is not None
        else "未知"
    )
    round_label = (
        f"第 {event.round_number} 回合"
        if event.round_number is not None
        else "回合数未知"
    )
    return (
        f"本次事件是“{_EVENT_LABELS[event.type]}”，话术类目是“{_CATEGORY_LABELS[category]}”。\n"
        f"时间位置：{round_label}。\n"
        f"事件事实：{facts}。\n"
        f"当前地图：{map_label}。当前比分：{score_label}。\n"
        "请只根据这些已知事实说一句即时反应。"
    )


def check_output(text: str, *, max_chars: int) -> OutputChecks:
    """Record factual violations without filtering or regenerating the output."""
    return OutputChecks(
        exceeds_max_chars=_chinese_character_count(text) > max_chars,
        callout_terms=tuple(term for term in CALLOUT_TERMS if term in text),
        raw_curses=tuple(term for term in FORBIDDEN_RAW_CURSES if term in text),
    )


def run_bench(
    recording_path: Path,
    *,
    model: str,
    personality_style: PersonalityStyle,
    client: LlmClientProtocol,
    max_events: int,
) -> BenchResult:
    """Replay one recording and generate model lines for every selected event."""
    if max_events < 1:
        raise ValueError("max_events must be at least 1")
    snapshots = load_recording(recording_path)
    configuration = load_config()
    replay = replay_commentary(
        snapshots,
        configuration.events,
        configuration.policy,
        personality_style=personality_style,
    )
    selected = tuple(
        disposition
        for disposition in replay.dispositions
        if disposition.decision.selected
    )
    chosen = selected[:max_events]
    length_statistics = commentary_length_statistics(personality_style)
    community_lore = _COMMUNITY_LORE_PATH.read_text(encoding="utf-8")
    snapshot_by_ts = {snapshot.ts: snapshot for snapshot in snapshots}
    events: list[BenchEvent] = []

    for disposition in chosen:
        event = disposition.decision.event
        if disposition.utterance is None:
            raise ValueError(f"selected event {event.id} has no template utterance")
        snapshot = snapshot_by_ts.get(event.ts)
        category = commentary_category(event)
        attempts = tuple(
            _attempt_completion(
                variant,
                event=event,
                category=category,
                snapshot=snapshot,
                personality_style=personality_style,
                model=model,
                max_chars=length_statistics.p90,
                community_lore=community_lore,
                client=client,
            )
            for variant in ("bare", "styled")
        )
        events.append(
            BenchEvent(
                event=event,
                category=category,
                template_text=disposition.utterance.text,
                map_name=snapshot.map_name if snapshot is not None else None,
                score_ct=snapshot.score_ct if snapshot is not None else None,
                score_t=snapshot.score_t if snapshot is not None else None,
                attempts=(attempts[0], attempts[1]),
            )
        )

    return BenchResult(
        recording_path=recording_path,
        requested_model=model,
        personality_style=personality_style,
        run_timestamp=datetime.now(timezone.utc),
        snapshot_count=len(snapshots),
        detected_event_count=len(replay.dispositions),
        selected_event_count=len(selected),
        truncated_event_count=max(0, len(selected) - len(chosen)),
        length_statistics=length_statistics,
        events=tuple(events),
    )


def render_report(result: BenchResult) -> str:
    """Render one blind Markdown comparison, revealing variants only at the end."""
    label_by_variant = _blinded_labels(result.blinding_seed)
    variant_by_label = {label: variant for variant, label in label_by_variant.items()}
    successful_results = tuple(
        attempt.result
        for event in result.events
        for attempt in event.attempts
        if attempt.result is not None
    )
    actual_models = sorted({item.model for item in successful_results})
    providers = sorted(
        {item.provider for item in successful_results if item.provider is not None}
    )
    stats = result.length_statistics
    lines = [
        "# M3-T1 话术评测报告",
        "",
        "## 这次跑了什么",
        "",
        f"- 录制文件：`{result.recording_path}`",
        f"- 处理快照：{result.snapshot_count}",
        f"- 检测到事件：{result.detected_event_count}",
        f"- 策略入选事件：{result.selected_event_count}",
        f"- 本次实际评测：{len(result.events)}",
        f"- 因 `max_events` 截断：{result.truncated_event_count}",
        f"- 性格：`{result.personality_style}`",
        f"- 请求型号 ID：`{result.requested_model}`",
        f"- 上游实际返回型号：{_join_or_unavailable(actual_models)}",
        f"- 上游实际服务商：{_join_or_unavailable(providers)}",
        f"- 运行时间戳：{result.run_timestamp.isoformat()}",
        (
            "- 模板汉字数（去除占位符）："
            f"最小 {stats.minimum} / 中位 {_format_number(stats.median)} / "
            f"P90 {stats.p90} / 最大 {stats.maximum}"
        ),
        "",
        "## 逐条对照",
        "",
    ]
    if not result.events:
        lines.extend(("（没有策略入选事件。）", ""))
    for index, event_result in enumerate(result.events, 1):
        event = event_result.event
        lines.extend(
            (
                f"### 事件 {index} —— {_EVENT_LABELS[event.type]}",
                "",
                f"事实：{_facts_sentence(event, event_result)}",
                "",
                f"- 模板：{event_result.template_text}",
            )
        )
        attempts = {attempt.variant: attempt for attempt in event_result.attempts}
        for label in ("甲", "乙"):
            attempt = attempts[variant_by_label[label]]
            lines.append(f"- {label}：{_attempt_text(attempt)}")
            lines.append(f"- {label}计量：{_attempt_metrics(attempt)}")
        lines.append("")

    lines.extend(("## 数字汇总", ""))
    for label in ("甲", "乙"):
        variant = variant_by_label[label]
        attempts = tuple(
            attempt
            for event in result.events
            for attempt in event.attempts
            if attempt.variant == variant
        )
        lines.extend(_summary_lines(label, attempts))
    lines.extend(
        (
            "## 变体对照表",
            "",
            f"固定打乱种子：`{result.blinding_seed}`",
            "",
            f"甲 = `{variant_by_label['甲']}`；乙 = `{variant_by_label['乙']}`",
        )
    )
    return "\n".join(lines) + "\n"


def write_report(result: BenchResult, output_path: Path) -> None:
    """Write the rendered report to the explicitly requested location."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(render_report(result), encoding="utf-8")


def _attempt_completion(
    variant: PromptVariant,
    *,
    event: GameEvent,
    category: CommentaryCategory,
    snapshot: GameSnapshot | None,
    personality_style: PersonalityStyle,
    model: str,
    max_chars: int,
    community_lore: str,
    client: LlmClientProtocol,
) -> BenchAttempt:
    try:
        result = client.complete(
            model=model,
            system_prompt=build_system_prompt(
                variant,
                personality_style=personality_style,
                category=category,
                max_chars=max_chars,
                community_lore=community_lore,
            ),
            user_prompt=build_user_prompt(
                event,
                category=category,
                map_name=snapshot.map_name if snapshot is not None else None,
                score_ct=snapshot.score_ct if snapshot is not None else None,
                score_t=snapshot.score_t if snapshot is not None else None,
            ),
            max_tokens=MAX_COMPLETION_TOKENS,
            temperature=TEMPERATURE,
        )
    except LlmError as error:
        context = []
        if error.status_code is not None:
            context.append(f"HTTP {error.status_code}")
        if error.latency_seconds is not None:
            context.append(f"{error.latency_seconds:.3f}s")
        suffix = f"（{'，'.join(context)}）" if context else ""
        return BenchAttempt(
            variant=variant,
            result=None,
            error=f"{error}{suffix}",
            checks=None,
        )
    return BenchAttempt(
        variant=variant,
        result=result,
        error=None,
        checks=check_output(result.text, max_chars=max_chars),
    )


def _chinese_character_count(text: str) -> int:
    return len(_HAN_CHARACTER_PATTERN.findall(_remove_placeholders(text)))


def _remove_placeholders(text: str) -> str:
    return _PLACEHOLDER_PATTERN.sub("", text).strip()


def _display_value(value: Any) -> str:
    if value is None:
        return "未知"
    if isinstance(value, float):
        return f"{value:.3f}".rstrip("0").rstrip(".")
    return str(value)


def _display_fact(key: str, value: Any) -> str:
    if key == "method" and isinstance(value, str):
        return WIN_METHOD_LABELS.get(value, "未知获胜方式")
    return _display_value(value)


def _facts_sentence(event: GameEvent, item: BenchEvent) -> str:
    facts = "，".join(
        f"{_FACT_LABELS.get(key, key)} {_display_fact(key, value)}"
        for key, value in event.facts.items()
    ) or "没有更多可用事实"
    map_label = item.map_name or "未知地图"
    score_label = (
        f"CT {item.score_ct}:{item.score_t} T"
        if item.score_ct is not None and item.score_t is not None
        else "比分未知"
    )
    return f"{facts}；{map_label}；{score_label}"


def _blinded_labels(seed: int) -> dict[PromptVariant, str]:
    variants: list[PromptVariant] = ["bare", "styled"]
    random.Random(seed).shuffle(variants)
    return {variants[0]: "甲", variants[1]: "乙"}


def _attempt_text(attempt: BenchAttempt) -> str:
    if attempt.result is not None:
        return attempt.result.text
    return f"调用失败：{attempt.error}"


def _attempt_metrics(attempt: BenchAttempt) -> str:
    if attempt.result is None:
        return "无成功计量"
    usage = attempt.result.usage
    checks = attempt.checks
    assert checks is not None
    return (
        f"延迟 {attempt.result.latency_seconds:.3f}s；"
        f"输入 token {_optional_number(usage.prompt_tokens)}；"
        f"输出 token {_optional_number(usage.completion_tokens)}；"
        f"花费 {_cost_label(usage.cost_usd)}；"
        f"超长 {'是' if checks.exceeds_max_chars else '否'}；"
        f"点位词 {_terms_label(checks.callout_terms)}；"
        f"未替代脏字 {_terms_label(checks.raw_curses)}"
    )


def _summary_lines(label: str, attempts: Sequence[BenchAttempt]) -> list[str]:
    successes = tuple(attempt for attempt in attempts if attempt.result is not None)
    failures = len(attempts) - len(successes)
    latencies = tuple(
        attempt.result.latency_seconds
        for attempt in successes
        if attempt.result is not None
    )
    prompt_tokens = tuple(
        attempt.result.usage.prompt_tokens
        for attempt in successes
        if attempt.result is not None
        and attempt.result.usage.prompt_tokens is not None
    )
    completion_tokens = tuple(
        attempt.result.usage.completion_tokens
        for attempt in successes
        if attempt.result is not None
        and attempt.result.usage.completion_tokens is not None
    )
    costs = tuple(
        attempt.result.usage.cost_usd
        for attempt in successes
        if attempt.result is not None and attempt.result.usage.cost_usd is not None
    )
    checks = tuple(attempt.checks for attempt in successes if attempt.checks is not None)
    latency_label = (
        f"P50 {_percentile(latencies, 0.50):.3f}s / "
        f"P95 {_percentile(latencies, 0.95):.3f}s / 最大 {max(latencies):.3f}s"
        if latencies
        else "上游未提供"
    )
    cost_label = (
        f"${sum(costs):.6f}"
        if successes and len(costs) == len(successes)
        else "上游未提供"
    )
    return [
        f"### {label}",
        "",
        f"- 调用：{len(attempts)}；成功：{len(successes)}；失败：{failures}",
        f"- 延迟：{latency_label}",
        f"- 平均输入 token：{_average_or_unavailable(prompt_tokens, len(successes))}",
        f"- 平均输出 token：{_average_or_unavailable(completion_tokens, len(successes))}",
        f"- 总花费：{cost_label}",
        f"- 超出字数上限：{sum(item.exceeds_max_chars for item in checks)}",
        f"- 命中点位词：{sum(bool(item.callout_terms) for item in checks)}",
        f"- 命中未替代脏字：{sum(bool(item.raw_curses) for item in checks)}",
        "",
    ]


def _percentile(values: Sequence[float], quantile: float) -> float:
    ordered = sorted(values)
    return ordered[math.ceil(len(ordered) * quantile) - 1]


def _average_or_unavailable(values: Sequence[int], success_count: int) -> str:
    if not values or len(values) != success_count:
        return "上游未提供"
    return f"{statistics.fmean(values):.1f}"


def _format_number(value: float) -> str:
    return f"{value:g}"


def _optional_number(value: int | None) -> str:
    return str(value) if value is not None else "上游未提供"


def _cost_label(value: float | None) -> str:
    return f"${value:.6f}" if value is not None else "上游未提供"


def _terms_label(values: Sequence[str]) -> str:
    return "、".join(values) if values else "无"


def _join_or_unavailable(values: Sequence[str]) -> str:
    return "、".join(f"`{value}`" for value in values) if values else "上游未提供"


def _positive_int(value: str) -> int:
    parsed = int(value)
    if parsed < 1:
        raise argparse.ArgumentTypeError("必须至少为 1")
    return parsed


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="离线评测 CS2 模板与大模型话术")
    parser.add_argument("--replay", type=Path, required=True, help="GSI JSONL 录制文件")
    parser.add_argument("--model", required=True, help="OpenRouter 型号 ID（无默认值）")
    parser.add_argument("--out", type=Path, required=True, help="Markdown 报告输出路径")
    parser.add_argument(
        "--personality",
        choices=("brother", "caster"),
        default="brother",
        help="要评测的现有性格",
    )
    parser.add_argument(
        "--max-events",
        type=_positive_int,
        default=20,
        help="最多评测多少个策略入选事件",
    )
    return parser


def main(argv: Sequence[str] | None = None) -> int:
    """Run the offline benchmark without exposing credentials in arguments."""
    args = _build_parser().parse_args(argv)
    try:
        client = OpenRouterClient.from_env()
    except LlmError as error:
        print(f"错误：{error}", file=sys.stderr)
        return 2
    try:
        result = run_bench(
            args.replay,
            model=args.model,
            personality_style=args.personality,
            client=client,
            max_events=args.max_events,
        )
        write_report(result, args.out)
    finally:
        client.close()
    print(f"评测报告已写入：{args.out}")
    return 0


if __name__ == "__main__":
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8")
    if hasattr(sys.stderr, "reconfigure"):
        sys.stderr.reconfigure(encoding="utf-8")
    raise SystemExit(main())

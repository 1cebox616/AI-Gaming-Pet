...
class EvidenceStore:
    ...
    def new_evidence_id(self, root_capture_id: str, source: str) -> str:
        if not root_capture_id.startswith("f") or not root_capture_id[1:].isdigit():
            raise ValueError("root_capture_id must use the form f<sequence>")

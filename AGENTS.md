# AGENTS.md
最后更新：T7.8 限流冷却已下发（B-T3 的前置）；B-T5 通过，三个本地传感器齐备；新增 TD-48/49
...
**现在**：M5-B「跟得上」；**三个本地传感器全部就位**（OCR `ccd06c6`、场景指纹与命名 `a6e8881`、鼠标聚合 `f71c865`）；**T7.8 限流冷却执行中**（产品负责人拍板先行），其后 B-T3 游戏初始化。详见 §6.2。
...
【后续新增】[games.generic.belief]（笔记员周期、策略参数）、[games.generic.init]（初始化预算与代表帧数）、
              [llm.profiles.notetaker]（文本档）。深线新用途优先复用 scene_fingerprint_verifier 档位，
              确有不同需求（更高 max_tokens 等）再新建，避免一个用途一个档位的配置膨胀
```
...
- **D-38 场景指纹（感知哈希）是唯一进第一版的本地视觉特征；v1 只做全帧、整屏 UI 状态。**
  ...
  **升格 = 稳定 + 被命名为界面（B-T2-4 后修订）**：过稳定门 → 命名 → 命名结果不是"普通游玩画面"即上卡。
  **驻留门（原 8 秒）降级为记录字段，不再作门**——它是"这是不是界面"的代理指标，而命名模型能直接回答那个问题（同 F-20 的形状）。
  原规则的实测代价：饥荒的主菜单、角色选择、加载界面三个都被命名却因驻留不足没上卡，于是每个会话重复调用一次模型。
  访问次数与驻留时长都只记录，供 B-T3 用。
...
| 4 | **B-T3 游戏初始化（在线渐进学习器）** | 首次遇到游戏时在线、渐进、不在关键路径上：主菜单期做一次货架一查询（每游戏一次、缓存）并深读主菜单簇；游玩中每出现新簇补一次深读（新建 vision_deep 档位：最强模型、高 token、结构化输出）得场景标签与 HUD 区域候选；OCR 稳定行归 ui:rN 槽位与语义角色候选；动作键事件后短窗口（待实测）内簇跳变 → 键位候选，计数印证；货架一事实以 source=init、modality=inferred 入卡。卡持续变厚，下一局直接装载。深读调用原语在此建成，供 B-T9 复用。录像重放只是验收台。每游戏首局约 10–20 次深读 | **第一场大考**：四张游戏卡逐字段判场景标签、HUD 槽位角色、键位；货架一事实有无越界（剧情类出现即打回）；卡里没有任何预置内容 | B-T2、B-T4 |
...
24. 中心加权 margin：准心永远在画面中心，中心块 margin 压低、边缘抬高，提高"准心里出现小目标"的检出率。数值须用含此类事件的录像经 --replay 校准。
    偿还：有合适录像时
25. 429 冷却与错误元数据（T7.8，**执行中**）：客户端保留上游错误类型与错误码、Retry-After、请求标识；
    **按档位隔离**的派发级冷却 + 指数退避（**非重试**，丢帧教义不变）；限流次数、冷却时长、被冷却丢弃的请求数
    进会话统计与评测报告。T7.7 曾因缺元数据无法归因 53 次 429；B-T2-5 的跑批被打断（TD-47）。偿还：本轮
...
46. "普通游玩画面"的判定是硬编码中文词表（`is_ordinary_gameplay_label`），见 F-23。四段录像上全对，
    但跨游戏必然漏判。偿还：**B-T3 顺带**——命名合同加一个受控字段让模型直接答"这是界面还是游玩画面"，
    词表降级为兜底；改动波及提示词与 payload，与 B-T3 的深线改动一次付清
47. 场景命名的在线跑批遭遇上游间歇 429，正式证据由多次独立会话的成功判决合并重放而成（每会话内未重试，

...
- M3-T5 信号密度重构：删除【本回合】汇总段、时间线增补五项、
  【刚刚】与【全场】降噪、reading.md 与 inference.md 重写、
  修复点位词黑名单对武器名的误报，然后复测读卡准确率。已下发
...
5. 点位词黑名单 `CALLOUT_TERMS` 含 `A1`，会以子串方式命中武器名 `M4A1-S`，
   M3-T4 报告中出现 4 次误报。该黑名单将来要当运行时闸门用，误报会拦掉正常输出。
   偿还时机：M3-T5

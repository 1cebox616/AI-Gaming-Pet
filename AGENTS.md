...
未实现（M3 任务序列，滚动细化）：
- M3-T8.10 用开火区间区分死亡类型：还手了没有、打空了没有。已下发
...
**场景标注是事实层与文风层之间的唯一接口。** 代码在事件卡上标出确定性场景
（残血击杀、白着打、踩火杀、一发命中、被补、白给等），该标注有两个用途：
1. 作为事实约束，模型必须表达出这个场景；
2. **作为语料选择键**——代码据此从分场景语料库中取出对应部分注入提示词。
因此模型永远只看到与当前场景相关的语料，不需要自己从上百条里挑。
这是 M3-T6.6 那条经验的延伸：**收窄模型的判断范围会同时提升准确率、
降低成本、降低延迟**；让模型自己挑语料，就是把一个它不擅长的判断任务塞回去。
标签必须收敛为可枚举的 `SCENE_TAGS` 常量，否则语料库无法与之对齐。

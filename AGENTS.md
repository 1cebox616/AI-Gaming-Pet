# AGENTS.md
最后更新：M5-B「传感器先行、快线最后重设计」；B-T4 OCR 已下发（三阶段：选引擎 → 最简单遍式 → 按需加机制）；新增 D-44、D-45、TD-31
...
**现在**：M5-B「跟得上」；B-T1 已通过；**下一个是 B-T4 OCR 接管线**（传感器先行，快线重设计排到最后）；T7.8（429 冷却）并行。详见 §6.2。
...
- **D-45 OCR 是常驻传感器，按延迟预算选型，不默认绑定某个引擎。** 目标：中位 ≤100ms、P90 ≤250ms（D-43 的分配），
  在此约束下取召回最高者。评估口径对所有引擎相同：同一批 1080p 帧、同一份人工真值、同时报中位/P90/召回/精确率/常驻开销。
  召回（找到的行 ÷ 画面上实际存在的行）比精确率优先——漏读的字永远不会进信念层，错读的字还能被 ui_text 策略按跨帧一致性驳回。
  选型按 a→b→c 实测，不靠读文档推断（F-12）。允许用 GPU，但受 D-39 硬预算约束，且必须先证明 CPU 档不达标。
...
### 5.4 本地 OCR（M5-T7.10 / T7.10b）

- WinRT OCR 中文可用；单帧中位 35–76ms；**640 宽素材零输出，分辨率是硬门槛**（1080p 录像可用，早期 640 录像不可用）
- 两遍式：首遍留出召回 85.2%（58ms），+ 彩色二次 96.3%（247ms）；灰度二次与双通路无增益。朴素版超预算，
  选择性二次 OCR 原型（持久工作进程 + 缓存差分 + 预算）已落地待接管线（B-T4）
- WinRT 无置信度、多版本可互相矛盾：OCR 输出是**证词不是事实**，稳定性由 D-34 的策略按跨帧一致性裁决
...
| 1 | **B-T4 OCR 接管线**（执行中，三阶段） | **4a 选引擎**：先建 40 帧人工真值集（TD-31），再横向实测现任 WinRT、Windows AI TextRecognizer（若可用）、RapidOCR/PP-OCR ONNX（CPU；DirectML 仅在 CPU 不达标时试），同口径量中位/P90、召回、精确率、常驻开销 → **停下等拍板**。**4b 最简单遍式**：选定引擎后全帧、单遍、无 crop、无预算、无爬坡接进 adapter，只保留行差分（证据语义，微秒级）；以 source=ocr 进证据流；量准确率与延迟 → **停下看数据**。**4c 按需加机制**（条件）：仅当 4b 超标才加，每加一项报告换来多少毫秒/多少召回；若 4b 达标则删除 ocr_selective.py 中未被使用的 crop/预算/爬坡代码。全程**不碰快线提示词**（D-44）。放宽 models.py/render.py：新 kind、同帧多条同 kind。顺带从重放的 evidence.jsonl 算出**输出解码速率**补进 §5.3 | 4a 读对照表（含漏读/错读样例）拍板选型；4b 重放四段 1080p，抽查 OCR 条目与画面对得上、中位耗时是否达标；4c 只在必要时发生 | B-T1 |
...
31. OCR 真值集缺失：现有 OCR 数据只有引擎自报的行，没有人工标注的"画面上实际有哪些行"，因此召回是估算。
    偿还：B-T4 建 40 帧真值集（四段录像各 10 帧，覆盖 HUD 数字、字幕、菜单、密集面板），入 data/generic/ocr-truth/ 作回归资产

...
最后更新：W-T0a 验收通过（`1cabed3`），W-T0b 下发；M5-B 重定案为「开放世界观察与记忆层 v1」在分支执行；B-T6 / B-T7 / B-T8 作废，由 W-T 序列取代
...
**W-T0a 观察协议模块已验收**（`1cabed3`：`core/belief/protocol.py` 唯一语法源、`frame_observation` 证据 kind，64 项协议测试 + 全量通过，契约破坏测试复现）。
**当前任务：W-T0b 抽样与标注工具（执行中）**，见 §2.2。
...
/backend/docs/               产品负责人可读的流程说明（ocr-pipeline.md 等）；coding agent 按规格要求生成，
                             只描述现状不写路线图；与 AGENTS.md 冲突时以 AGENTS.md 为准
...
| 序 | 任务 | 内容 | 产品负责人怎么验 | 依赖 |
|---|---|---|---|---|
| **0** | **W-T0 观察语言（拆五段，逐段敲定）** | **0a ✅ `1cabed3`** 协议与解析器：`protocol.py` 唯一语法源、POS 两种编码、`frame_observation` 证据 kind 与信封；遗留一处例子措辞（`A o1 condition stationary` 与 `V o1 moving` 同框自相矛盾），W-T0c 拼提示词时顺手改。**0b（执行中）** 抽样与标注工具：按机械信号分层抽 60 帧（上传帧集合内）、标注网页（原图显示 + 896 模型视角切换；只在 896 下认不出的对象另做标记、不计入召回分母）、truth.json。**0c** 跑台 + 无真值指标：当前模型上先测 POS 编码 × 叠加网格（2×2）再测提示词语言，三次重复，预注册判据。**0d** 模型横评：先探针（端点 / 推理关闭参数 / 严格 schema 支持 / 429），再在选定格式上跑 5 款，严格 schema 可用的模型加"紧凑 JSON"臂。**0e** 真值指标、总报告、拍板；之后 0f 接生产 adapter。**全程不碰归并器** | 0a：pytest + 读生成的格式段；0b：自己标 60 帧；0c / 0d：读表；0e：拍板协议、POS、语言、模型 | — |
...
scene_verified（source="deep"，B-T2-4 起）—— 场景的命名与跨会话复核。payload 至少含：被命名的会话簇 id、
    label、annotation、modality（observed / inferred / uncertain）、所看帧的 root_capture_id 列表、模型名与花费，
...
knowledge —— 货架一，整体替换、带出处（B-T3b 起；顶层 genre / perspective 并入此处）
    content（V4 合同的 JSON 原样，见 D-37；通过完整校验的才写入；**不含键位，不留键位占位字段**）,
...
（keybinds{} 与 view_constants 已移除。玩家实际键位将来由设置菜单的视觉识别获得并带画面证据，见 TD-54；
  在此之前卡上没有任何键位数据，也不预留空字段）
init（initialized_at, source_recordings, version）
```

**卡里不存图片**（D-36）：场景的持久表示是 `label + annotation + representative_hash`。
代表帧路径只出现在离线评测报告里，不进卡。跨会话仍按 D-48 用哈希距离找候选。
...
- **D-45 OCR 是常驻传感器；判据是"跟得上节奏 + 不跟游戏抢资源 + 召回最高"，不是一个抽象毫秒数（4a 后修订）。**
  ...
  2. **不抢资源**：平均 CPU 核数 ≤ 0.5 核；**且游戏运行中实测帧时间与 1% low 的劣化不可感知**（受 D-39 约束）。
     **RSS 与峰值核数都只记账不作门**（4b 后修订，理由见 F-20）——它们是代理指标，真正要防的"打扰游戏"
     由帧时间直接回答；RSS 的实际风险落在低配机器与安装包，归 M10（TD-36）。CPU 用**每次 OCR 的进程 CPU 核心秒数**度量（调用前后 cpu_time 差值），
     1 Hz 下这个数就等于平均占用核数——精确、可复现，不用任务管理器的百分比估算。
     **实机对照必须在同一段会话内交错开关，不得两段分跑**：游戏负载自身的波动远大于被测效应，
     分段 A/B 测出的差异主要是关卡差异（F-20）
...
- **D-37 游戏卡：每游戏一份跨会话持久记忆，在线渐进生成，零预置。** 首次遇到游戏时边玩边学、不在关键路径上：主菜单期做货架一查询
  ...
  **提示词合同 V4**（严格 JSON Schema，扁平，B-T3b 实测后修订）：genre[]（≥1，必填）、perspective、summary（必填）、
  core_gameplay（目标、循环、主要系统，一段）、game_structure（对局/回合/远征/持续世界）、setting_and_background（只写基础设定，不剧透）、
  release_and_service_status。除 genre 与 summary 外均允许 null——**不确定就留空，不许猜**。
  **键位已整体移出货架一**（产品负责人定案，F-24）：默认键位不是可靠的公开事实（一动作多键、随版本/设备/模式变、玩家自改），
  且作为必填项时曾让守望先锋整份有效知识连续三次被 Shift 歧义否决。玩家实际键位归货架二，见 TD-54。
  **明确不问**：键位、HUD 惯例（归 HUD 识别线）、社区术语、手柄、剧情/结局/角色命运、关卡解法、地图与隐藏内容。
  **时效（B-T3b2.5）**：代码为每次请求机械注入当前 UTC 核查日期（模型不猜今天是几号）；提示词要求优先官方页面、
  单独核查运营状态、区分"网页发布日期"与"事件日期"、"预计/计划"的日期已过必须继续查、确认不了就 null。
  实测把杀戮尖塔 2 从"预计 2025 年抢先体验"纠正为"2026-03-05 已开启"。**这是提示词层面的降低概率，不是代码级保证**（TD-55）。
- **D-38 场景指纹（感知哈希）是唯一进第一版的本地视觉特征；v1 只做全帧、整屏 UI 状态。**
  ...
  **不预设任何场景分类**——不是每个游戏都有背包或地图；簇是发现出来的、无名的，标签由 B-T3 深读去填；
  从不稳定成簇的帧就是"默认场景"（游玩中），这不是失败而是设计。
  **v1 边界（产品负责人划定）**：只识别全屏、稳定、最容易辨认的画面状态。弹窗式背包（局部固定、其余是游戏画面）
  与"边框固定、中央一直变"的地图 UI 属于**局部指纹**问题，留给后续改型；自然的 v2 路径是在检测器已有的 9×16 块网格上
  逐块取哈希（弹窗 = 一组块匹配已知模式而其余块自由变化；地图 = 边缘块稳定而中央块变化），是同一机制的扩展，
  不是重设计。v1 不为它预留代码。
  **阈值、哈希变体、稳定时长都从数据来**（TD-28）：固定 UI 也常带动画，全帧哈希会抖；噪声底用"检测器判 no_change 的相邻对"
  或人工区间估，不用全体相邻对（F-21）。**已定：pHash / 64 位 / 汉明阈值 8**（噪声底 P95 = 8，平台 8–15）。
  **指纹看全部轮询帧，检测器只管上传（B-T2-2 后定案，F-22）**：检测器去重掉的正是"画面没变"的帧，而指纹要找的
  就是"画面没变"——只喂它选中帧，固定界面永远凑不齐稳定条件（四段录像四张空卡）。指纹微秒级，看全部帧零成本；
  证据仍只挂在选中帧上（转场必然触发检测器，所以转场帧天然是选中帧），稳定计数在所有帧上累积。
  **升格 = 稳定 + 已命名（B-T3 前再修订）**：过稳定门 → 命名 → 即上卡。驻留门（原 8 秒）降级为记录字段。
  "普通游玩画面"的过滤**已整体移除**（产品负责人定案）：它靠一张硬编码词表猜模型措辞（F-23），与其修成受控字段
  不如不做——偶尔稳定下来的游玩簇（如卡牌游戏的"战斗场景"）也有名字、人读得懂，留在卡上无害。
  访问次数与驻留时长都只记录。
  **生产参数定案（B-T2-3）：pHash64 / 汉明 8 / 连续稳定 4 秒 / 累计驻留 8 秒。** 四段录像 1632 个原始候选收敛到
  12 个正式簇、8 个写卡簇，人工复核**零普通游玩误收**。反事实对照：放宽到 H10/3s/6s 召回升到 14 但收进 2 个普通战斗画面；
  收紧到 H5 低于噪声底，画面被切碎且仍有误收。**选零误收**——漏掉的界面下次会话还能补，写错的会污染持久记忆（D-35）。
  **视觉簇只回答"像不像"，回答不了"是不是同一个界面"**：守望先锋的空闲大厅与搜索中大厅跨簇汉明距离 20–24，
  任何全局阈值都并不了；这不是调参问题，是一个概念担了两种职责。语义归并见 D-49。
- **D-49 场景的命名由深线判定，代码执行；视觉层不做语义。v1 只命名，不归并。**
  "这是什么界面"是判断题，按 D-01 归模型、按 D-28 由代码执行。**触发时点是"某个视觉簇通过稳定门"**——
  独立能力，不依附于游戏卡初始化：一个簇稳定下来 → 发一次深线请求 → 拿到名称与一句话注释 → 代码验证后写卡。
  不在每帧热路径上，每个游戏一辈子只发生几次。
  **视觉变体与语义归并 v1 不做**（产品负责人定案）：当前需求只是"大致区分主要场景"，一个视觉簇对应一个场景条目即可。
  守望先锋的空闲大厅与搜索中大厅会成为两个条目，接受——它们各自有名字，人读得懂，下游也用得了。
  重开条件：卡上重复条目多到干扰使用，或 B-T3 / B-T8 明确需要"同一界面的多个外观"这一概念。
  届时再考虑 variants[] 与归并判决，届时也才考虑局部指纹（此前的重开条件仍然有效）。
  **本地布局签名（粗网格边缘、HSV 直方图、时序转移图）不做**：它们是让代码去做判断题，每一项都要新的人工标注集来定阈值，
  是在重建深线一次调用就能回答的东西。OCR 稳定文字行可以作为深线判断的证据一并附上，但不得自行按重叠度合并。
- **D-50 已命名场景每个会话复核一次；名字一旦定下，匹配时不许模型改写。** 跨会话不直接复用旧名：新会话里由 pHash 匹配到
  卡上已命名场景的稳定簇，连同卡上现名与注释一起交给核查模型问"是不是同一种界面"。**匹配则代码复用原名，不接受模型改写**
  ——这是名字稳定性的唯一来源（实测同一簇不同调用会给出"休闲比赛匹配界面 / 主菜单匹配界面 / 比赛搜索界面"三种说法）。
  不匹配且非 uncertain 才覆盖，且必须把旧名写进证据留痕；uncertain 时保留旧名并标记待复查。
  成本模型随之改变：不是"每个游戏一辈子几次"，而是**每会话每个稳定簇最多一次**（实测每次约 $0.0008、3.2 秒，异步不阻塞）。
  这条同时兑现了 D-48 想防的事——主菜单换皮时复核会发现不匹配。
- **D-51 模型调用按功能分线，每条线有自己的档位、提示词、触发、节奏与预算（产品负责人定案）。**
  "快线 / 深线"这对名字已经不够用：深线实际承担了三种完全不同的工作，节奏和提示词各异，共用一个名字只会让档位命名、
  预算与验收混在一起（B-T2-4 的档位被迫按用途改名就是征兆）。现在的四条线：

  | 线 | 做什么 | 触发与节奏 | 档位 |
  |---|---|---|---|
  | **帧观察线**（旧快线） | 连续快速的帧视觉识别，写证词 | 每个上传帧，发言链路在等 | `vision_fast`（改名留给 B-T8） |
  | **场景命名线** | 稳定簇是什么界面 | 簇过稳定门一次；每会话复核（D-50） | `scene_fingerprint_verifier` |
  | **游戏知识线** | 货架一：类型、视角、机制、UI 惯例、默认键位 | **游戏首次初始化一次；之后每会话联网核查/更新** | B-T3a 选型 |
  | **HUD 识别线** | 游玩画面上有哪些 HUD 元素、在哪、怎么读 | 持续：游玩期按节奏问，结果 append 到同一张卡 | B-T3c 定 |

  游戏知识线与 HUD 识别线**必须是两条独立路线**：前者每会话一次、答的是公开知识；后者持续、答的是亲眼所见（货架二）。
  两者只是写进同一张游戏卡。将来归并器的待查深读（原 B-T9）是第五条线，届时再命名。
  "深线"作为总称保留（= 所有不在反射路径上的调用），不再指任何单一档位。
- **D-48 会话内的指纹索引是权威，游戏卡只是先验。** 线上游戏主菜单会定期整体换皮，老卡认不出新菜单是正常的。
  每个会话从零建自己的场景索引；卡只给会话簇提供"可能是同一个"的候选，**装卡不绑定、不改会话簇 id**；
  写卡时才保守合并：有候选且达标就并入（累计 sessions_seen），否则新场景；老场景 last_seen 停在上次，不删。
  与 D-33 同一条道理：宁可拆，不可错合。淘汰与衰减留给 M6。
...
- **D-40 两货架规则。** 分界线是"玩前可得 vs 玩后所见"。货架一·开箱即知（可 web search，每游戏一次，本地缓存）：类型、世界观一句话、
  ...
  结局、地图与世界内容，**以及玩家实际键位与 HUD 布局**（B-T3b 后修订：默认键位不再算货架一，见 D-37 / TD-54）。游玩中允许由观察触发的按需查询：屏幕上出现了不认识的名词才去查那个词条，按词条缓存、限频。
...
**生产管线（B-T4b，v6 tiny / OpenVINO / 2 线程 / det1280 / 内存直传）**：
- 40 帧 anchor 严格召回 **270/324 = 83.3%**，与 4a-2 探针同配置一致——**生产路径与探针路径无漂移**
- 实机（文明 6，每档 2 分钟）：1/2/4 线程的 OCR P90 = 239.0 / 183.6 / 168.5ms，CPU = 0.192 / 0.222 / 0.318 核；
  默认 2 线程 64 次调用零积压
- RSS 稳态 682MB / 峰值 886MB。**模型本身只有 6.03 MiB（FP32）**——内存几乎全是运行时与内存池开销，
  不是权重；量化省不了 RSS（TD-36 据此修正）
- **帧时间对照无效**：关闭档均值 8.609ms / 1% low 38.36fps，开启档 5.808ms / 76.43fps——开了 CPU 消耗反而快一倍，
  两段负载不可比。D-45 判据 2 的门**仍未建立**（TD-38）
...
**B-T3b 生产实测（V3 合同）**：3 个会话 12 次调用 $0.0198，零限流；守望先锋连续三次因键位歧义整份被拒（F-24）。
**V4 + 时效核查（3b2 / 3b2.5）**：四张卡全部 ok/refreshed，验收总花费 $0.036；杀戮尖塔 2 的过期"预计"被纠正。
...
"向右·大幅"，虽然净位移向右但实际扫了两个方向。对"画面变化是不是玩家自己造成的"这一用途无碍，
消费者出现后需复核（TD-48）。

**场景命名（B-T2-4，四段录像 12 个稳定簇）**：请求成功 12/12，代码接受 12/12，modality 全部 observed，
拒绝 0、uncertain 0；延迟均值 3.23 秒、P50 3.24、最大 3.77，12/12 在 6 秒内；总花费 $0.0096（每次约 $0.0008）。
早前用更大模型的同一批为 $0.0205。上传宽度 1920、每次最多 3 张（稳定段首/中/末）。
**名字不稳定**：同一簇不同调用出现过"休闲比赛匹配界面 / 主菜单匹配界面 / 比赛搜索界面"三种说法，语义一致措辞不同——
D-50 的"匹配时复用旧名、不许模型改写"是目前唯一的稳定性来源（TD-44）。

W-T0a：协议解析零模型、微秒级，无延迟数据；解码速率待 W-T0c。**观察语言 A/B（W-T0c / 0d）、归并器（W-T2）、轨迹（W-T4）、事件（W-T5）、词表（W-T6）、笔记员（W-T8）**起逐项填入。
...
24. 中心加权 margin：准心永远在画面中心，中心块 margin 压低、边缘抬高，提高"准心里出现小目标"的检出率。数值须用含此类事件的录像经 --replay 校准。
    偿还：有合适录像时
25. ~~429 冷却与错误元数据~~ 已随 T7.8 落地（`3ac5b8e`）：错误类型/码/Retry-After/请求 ID 保留并进统计；
...
36. OCR 内存偏重：实机稳态 682MB / 峰值 886MB，加前端固定 408MiB，整机超 1.1GB。
    **量化不是解法**——FP32 权重只有 6.03 MiB，RSS 的百倍差额来自 OpenVINO / OpenCV 的运行时与内存池
    （此前"量化省内存"的判断作废）。可试方向：OpenVINO 内存池配置、把 OCR 放进可回收的独立进程、或接受现状。
    偿还：M10 打包前评估。量化仍可为**体积与速度**单独评估，但不得再以省内存为由
37. `eval-reports/m5-b-t4a2/runs/` 混有 V1 与 V2 两轮的原始 JSON（V2 中途停止），不可用于逐帧复核；
    汇总 `report.md` 与 `metrics.json` 是完整 V1，可用。版本库已随 4b 清空 eval-reports，本地保留
38. **D-45 判据 2 的门仍未建立**：4b 的实机 A/B 两段分跑，测出"开 OCR 反而快一倍"，无效（F-20）。
    现有依据只有产品负责人的主观观察（游玩中无明显掉帧），据此接受现状继续推进，**但这不是实测**。
    偿还：**B-T10 真机验收时一并做**——同一段会话内交错开关 OCR（30 秒一换、总时长 ≥10 分钟、含至少一次
    场景切换），按桶对照帧时间与 1% low；建议用灰区一类第一人称游戏，回合制游戏的 AI 回合是巨大噪声源。
    **在此之前不得在任何文档或报告中宣称"OCR 对游戏无影响"**，只能说"未观察到明显掉帧，未实测"。
    若 B-T10 测出可感知劣化，回退手段已就绪：`[games.generic.ocr].enabled=false` 或降到 1 线程
39. OCR 引入 15 个新依赖（openvino、opencv-python、shapely、requests、tqdm 等），且把 numpy 从 2.5.2
    降到 2.4.6——**降级未申报**。安装体积与 M10 打包成本未测。`requests` 随 rapidocr 进入依赖树，
    虽然模型路径已显式钉死、运行时不下载，读屏模块的依赖树里有网络库仍需记账。
    偿还：B-T4b-2 报告安装体积与降级理由；test_layering 增加"OCR 模块不得 import 网络模块"的静态断言
40. ~~`RapidOcrWithoutClassifier` 覆写私有 API 无防线~~ 已加行为回归测试并钉住版本号 3.9.2（4b-2）。
    升级 rapidocr 时该测试失败是复核信号，不是修测试的信号。已销
41. `backend/memory/` 在测试阶段暂进仓库（产品负责人决定，便于对照验收）。它是跨会话用户数据。
    偿还硬线：**M10 分发前必须加入 .gitignore 并从版本库移除**；在此之前任何入库的卡不得含玩家身份信息
42. 场景区间真值：指纹阈值的监督复核需要人工标注的"这段是同一个全屏画面"区间（`data/generic/scene-truth/spans.toml`，
    产品负责人自行标注四段完整录像里的主菜单 / 加载 / 结算等明显全屏状态）。未标注前只有无监督曲线，默认值为临时值。
...
45. 场景命名档位是 flash 级模型（qwen3.8-flash），对"这是什么界面"够用（12/12 observed）。
    游戏知识线由 B-T3a 探针独立选型；HUD 识别线的档位在 B-T3c 自行验证，**不得默认沿用**任何一条线的档位
46. ~~"普通游玩画面"硬编码词表~~ 功能已由产品负责人决定整体移除（D-38），不修。已销
47. ~~场景命名跑批被 429 打断~~ 手段已就位（T7.8）；B-T3 跑批将首次带着限流统计验收。已销。原文：
    场景命名的在线跑批遭遇上游间歇 429，正式证据由多次独立会话的成功判决合并重放而成（每会话内未重试，
    符合丢帧教义）。这是 TD-25（429 冷却与错误元数据）第一次造成实际阻碍。
    偿还：TD-25 的优先级从"B-T10 前"提到"B-T3 之前或并行"——B-T3 的初始化要连续发十几次深读，没有冷却会更难受
48. 鼠标幅度分档是**全局固定的原始计数阈值**（<550 / 550–1599 / ≥1600），由四段录像的合并分布取 P33/P67 得来。
    但计数值随 DPI 与每个游戏的灵敏度设置而变，同一次"大幅转身"在不同游戏里的计数可以差几倍；
    且等频分桶让守望先锋 38.9% 的窗口都是"大幅"，标签的信息量被稀释。
    结构性解法是**按游戏学分布**（游戏卡正是每游戏的存储处）。偿还：B-T3 顺带在卡上记录该游戏的位移分布，
    或推迟到 B-T6 / B-T8 真正消费这个信号时按那个决策重新校准——**在有消费者之前不要先调**
49. 历史录像的输入探针 CSV 按原隐私设计只存绝对轴量，重建不出左右/上下符号，因此**离线重放的鼠标方向只能给
    "横向/纵向"**；四向符号只在实时生产路径上有。后果：B-T5 验收标准里"重放一段记得转了 90° 的片段核对方向"
    在现有素材上做不到，需要真机会话。偿还：B-T10 真机验收时顺带核对方向；或录一小段带方向的新探针素材（qwen3.8-flash），对"这是什么界面"够用（12/12 observed）。
    B-T3 要问的是 HUD 槽位语义角色等更难的问题，**不得默认沿用**：B-T3 需自行验证该档位是否胜任，
    不胜任则新建更强档位并在报告中给出对照
...
51. 游戏知识线的选型样本小且无正式真值：完整 15×3 矩阵跑在旧提示词上，V3 合同下每模型只有 8 次决赛；正式判卷表未填完，
    产品负责人基于后续对比直接拍板。选型可逆（只是档位配置），但**知识答案没有任何经人工核对的真值**。
    偿还：B-T3b-2 验收时产品负责人对四个有录像的游戏逐字段判一次 V4 合同的 knowledge（键位已不在其中）；不达标则换回候选二
...
54. **玩家实际键位靠设置菜单的视觉识别获得**（货架二）。（偿还时点中的 B-T8 读作 W-T0）边界：只表示实际观察到的绑定，必须引用画面/OCR 证据；
    ...
    在此之前卡上不预留键位字段，B-T5 的绝对角度接口保持 null
55. 游戏知识的时效只有提示词级保证：过期资料仍可能进卡（模型抓到旧页面且未继续核查）。代码级保证需要合同增加

# AGENTS.md
最后更新：货架一取消键位（合同 V4）；键位归货架二、由设置菜单视觉获取（TD-54）；B-T3b-2 已下发；新增 F-24
...
**现在**：M5-B「跟得上」；**三个本地传感器全部就位**（OCR `ccd06c6`、场景指纹与命名 `a6e8881`、鼠标聚合 `f71c865`）；**B-T3b-2 执行中**：按合同 V4（无键位）收尾游戏知识线，重生四张卡；之后 3c HUD 识别线。详见 §6.2。
...
**B-T3b 生产实测（V3 合同）**：3 个会话 12 次调用 $0.0198，零限流；饥荒、灰区、杀戮尖塔 2 初始化/刷新成功；
守望先锋连续三次因键位歧义整份被拒（F-24）——机制、延迟、成本结论有效，内容通过率不再作数，待 V4 重测。
...
| 4 | **B-T3 游戏初始化**（拆三段） | **3a 选型探针**（✅ `64c3043`）：Gemini 3.1 Flash Lite 仅联网，见 D-37 与 §5.6。**3b 游戏知识线**（`53e8b78` 机制就位；**3b-2 执行中**：合同改 V4 去键位、重生四张卡）：初始化一次 + 每会话联网核查/更新，写卡 `knowledge` 对象，失败保留上一份；短视图 512 UTF-8 字节；不注入帧观察线（那是 B-T8）。**3c HUD 识别线**：开放式——模型看游玩画面自述有哪些 HUD 元素、在哪、怎么读，持续 append 到卡；OCR 作旁证。键位印证、角度常数、游玩画面过滤均不做 | 3b：打开四张卡的 knowledge 段逐字段判，重点键位；3c：打开四张卡逐字段判 | B-T2、T7.8 |

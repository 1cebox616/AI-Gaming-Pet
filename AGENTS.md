...
- 语言模型：**经 OpenRouter 调用，OpenAI 兼容协议**。产品负责人无中国大陆手机号，
  无法注册阿里云百炼，因此不走厂商直连。**具体型号与上游供应商由 M3-T1 / M3-T2
  实测选定，任何代码与文档都不得写死型号名**，一律作为参数传入
...
- /backend/src/pet —— 后端源码包
    ...
    replay.py                录制回放工具与 CLI（复用线上事实/策略/话术实现）
    llm.py                   语言模型客户端（M3-T1 新增）
    bench.py                 离线话术评测台与 CLI（M3-T1 新增）
...
GameSnapshot（gsi.py）—— CS2 某一瞬间的状态，除 ts 外全部字段可为 None：
    ...
    round_wins（每回合获胜方式历史）,
    team, health, equip_value, round_kills, round_killhs, match_deaths,
    score_ct, score_t, ct_consecutive_round_losses, t_consecutive_round_losses,
    active_weapon

    规则：解析后无消费者的字段一律不得保留。M2-T9 已按此删除
    bomb / armor / helmet / flashed / smoked / burning / money /
    match_kills / match_assists / match_mvps / match_score 共十一个字段。
    后续里程碑需要哪个字段，那个里程碑再加回来。
...
回合号规则（必须只有一处实现，M1–M2 期间曾分叉过）：
    唯一实现是 gsi.human_round_number()，session.py 与 events.py 都必须调用它。
...
分层职责：
    ...
    replay      只做离线回放，绝不被线上链路引用

## 当前状态

当前里程碑：M3 已开始，M3-T1 已下发。

M3 目标：宠物在 CS2 里说的每一句话都由大模型当场生成，且比现有模板更像人话；
生成失败时回退模板不能变哑；花了多少钱在界面上能看见。
产品负责人的验证方式：打两局，听宠物说的话，判断是否比模板好听。
...
未实现（M3 任务序列，滚动细化）：
- M3-T1 离线评测台：跑通「录制 → 事件 → 提示词 → 模型 → 对照报告」，
  用「无样例 vs 全样例」两个提示词变体验证网感假设。已下发
- M3-T2 横向评测：多模型 × 多上游供应商 × 流式与非流式，
  产出 P50/P95 延迟与最终选型
- M3-T3 提示词与样例池正式模块化，双性格落地，输出闸门（长度、脏字、点位、不冒认）
- M3-T4 线上接入：异步生成、超时、失败回退模板、密钥配置
- M3-T5 成本计量与花费显示
- M3-T6 里程碑验收：真实对局实测与调优
- M4 打包分发，朋友可安装
...
M3 已拍板的四件事：
- **模板路径永久保留。** 模型调用失败（超时、返回不可用、欠费、断网）时回退模板，
  并在托盘菜单显示当前处于模板模式。宠物永不哑火。
  附带收益：模板是判断"模型有没有让宠物变好听"的唯一对照组
- **一次事件最多调用一次模型，绝不重试。** 这既是唯一的失控花费路径的堵口，
  也是正确的产品行为——重试一次就是再等两秒，那个情绪瞬间已经过去了。
  因此不设花费熔断，只做花费显示。产品负责人可接受的开发期预算是 ¥20/小时，
  按事件驱动与九成拦截率估算实际约 ¥0.2–0.65/小时，余量 30–100 倍
- **性格 = 不同的系统提示词 + 不同的样例池 + 各自的模板兜底。**
  用户自定义性格推迟到 M4 之后：那等于把提示词工程交给用户，质量不可控
- **模型输出必须有硬性字数上限。** 模板是手写的，长度已知（10–20 字，念完约 2 秒）；
  模型不受约束，40 字的句子念完要 5–6 秒，句子数没变但占用耳朵的时间翻倍。
  上限取现有 201 条语料字数分布的 P90，即用产品负责人自己的口味当基准

events.py 的 facts 使用中文标签是有意的：score_situation 取值为
"大比分领先 / 领先 / 追平 / 落后 / 大比分落后"。理由是这些 facts 的最终目的地
是中文提示词，转成英文枚举只是多一层翻译。约束：events.py 除这一组取值外
不得再引入任何展示性文本，尤其不得引入任何话术片段。

不做交火期禁言：原设计在玩家存活且回合进行中把门槛设为 75，理由是脚步声就是命。
产品负责人实测后推翻：及时的情绪反馈本身就是卖点，嫌吵可以用静音开关。
alive_priority_threshold 默认为 0，配置项保留。
M2 实测复核：当前话密度产品负责人认为没有问题，M3 不调整策略参数。
...
- 已验证的认知：大模型的默认输出风格和"客服体/教练体"高度重合。
  直接问它"生成一句 CS2 击杀解说"会得到"好枪，人数优势拿到了"这种毫无人味的句子。
  要得到"好枪兄弟好枪"，必须在提示词里塞大量对味的样例。
  M3-T1 的核心任务就是把这条认知从"相信"变成"测出来的数字"
...
语言模型：**尚无任何实测数据。M3-T1 与 M3-T2 的唯一目的就是产出这一节的内容。**
在拿到数字之前，任何关于"哪个模型更有网感""哪条线路更快""流式能不能救延迟"
的说法都是假设，不得写进设计。
...
- 大模型型号 ID 与上游供应商名一律作为参数传入，禁止在源码中写死默认值。
...
2. 后端测试套件 58.65 秒，其中约 58 秒是定时广播测试的真实等待。
   逼近既定的 60 秒阈值，应为这些测试注入时钟。M3 会继续加测试，
   届时若越过 60 秒必须先还这条
3. 三条语料事实检查是有限黑名单，只能拦截已枚举的词，不能证明完整语义规则。
   M3 之后语料不再是输出，这三条检查失去保护对象，
   必须从"测试时检查静态语料"改为"运行时拦截模型输出"。偿还时机：M3-T3
4. speech.py 用 powershell.exe -ExecutionPolicy Bypass -EncodedCommand 调用系统语音，
   该组合是恶意脚本的典型特征，杀毒软件可能拦截。偿还时机：M4 打包前必须验证
5. speech.py 每次朗读新建一个 PowerShell 进程用于合成。
   M2 发言频率提高后需实测评估是否改为常驻进程
6. requirements.txt 未区分运行依赖与测试依赖，测试链约占 13.9 MiB。偿还时机：M4
7. main.py 的跨域白名单只覆盖开发来源，打包后的实际前端来源尚未确认。
   偿还时机：M4 打包前必须复核，否则安装版会持续显示"连接失败"
8. 项目自身没有 LICENSE，pyproject.toml 也未声明 license。
   依赖许可证已全量核实，无 GPL/AGPL/非商用项。偿还时机：M4 打包前
9. pet.ts 同时承担宠物渲染与窗口拖动的发起逻辑。偿还时机：M6 更换外观时
10. 前端与 Rust 自动化测试覆盖很薄（Rust 仅 3 个纯函数测试，前端无测试）。
    暂不处理，若出现前端逻辑回归再引入最小测试手段
11. 启动时先 show() 再定位窗口，理论上存在瞬时跳位。两次审计以 1ms 轮询均未观察到
12. backend-bridge.ts 的 lastUtteranceId 去重并非规格要求，属防御性代码。观察中
...
8. 先看成熟产品如何解决同类问题，用已验证的模式，但按本项目规模裁剪，
   不照搬为更大规模设计的方案。
9. 涉及浏览器、操作系统 API、GPU 合成、网络链路这类底层行为时，

...
- /backend/src/pet —— 后端源码包
    ...
    llm.py                   语言模型客户端（业务无关）
    situation_card.py        把事实渲染成喂给模型的富卡文本
    bench.py                 离线话术评测台与 CLI
- /backend/tests             后端测试与 fixtures
- /backend/prompts/          系统提示词文件，**产品负责人可直接编辑，改完重启后端生效**，
                             coding agent 不得改动其中的措辞，只负责加载
...
分层职责：
    ...
    situation_card  只把已有事实渲染成中文文本，不新增任何判断、不访问网络
...
- M3-T2.5 数据清单脱敏、四项推导修正、时长类事实、炸弹与带包状态、移除失效探针。已下发
  ...
  关于原计划中的「复杂推导（依赖 3–5 个数据类型）」：经复查，该批条目不属于同一层，
  已按性质拆散，不单独设任务——
  残血击杀、武器大师、连续被闪压制属于"组合描述"，不写代码，
  由 M3-T3 的富卡报告验证模型能否自行组合；
  开局买枪窗口、回合节奏判断属于触发器，归 M3-T4；
  经济趋势需要跨回合状态，归 M3-T3 的对局级结构或 M3-T9；
  敌方经济与装备对比已确认数据层面不可得，永久不做。
  若 M3-T3 的报告显示模型无法可靠组合，届时再补一个任务，
  那时我们会知道具体缺哪几条，而不是现在凭空设计
- M3-T3 富卡组装 + 评测台升级（锁定上游服务商、系统提示词外置为可编辑文件、
  单变体运行）。与 M3-T1 的已提交报告做前后对照，不再做双变体盲测。已下发
- M3-T4 新增 `MatchSituation`（对局级累计）+ 新增四个事件（bomb_planted / bomb_defused / round_started / low_health
  / heavy_damage，后两者需互相去重）+ 改造后的 player_flashed（判据为被闪时长）
  + policy 扩展。**事件与策略必须同批改，不得先落地事件再补策略**

  `MatchSituation` 契约草案（M3-T4 落地，此处先定规矩防止再次遗失）：
  它是继 GameSnapshot（瞬时）、RoundSituation（回合级）之后的第三层，
  装"这一局到目前为止发生过什么"。
  **只装快照表达不了的历史，绝不复制已有字段**——
  全场 K/A/D/MVP、比分、连败轮数、每回合胜负方式都已在 GameSnapshot 上。
  它该装的是：本局事件流水（复用 policy 已有的优先级排序，供 M3-T9 记忆回响直接读取）、
  每回合的经济与结果摘要（支撑"连着第三个穷局"）、换边标记。
  它排在 M3-T4 而非 M3-T3，因为 M3-T3 的富卡不需要它，
  提前建就成了没有消费者的结构，违反本文件的字段生命周期规则
- M3-T5 事实闸门 + 地图战术知识接入
...
**富卡渲染的四条硬规矩**（situation_card.py）：
1. **未知的字段直接省略，不得写"未知"或用 0 顶替。**
   写"血量：未知"等于邀请模型对未知发表意见
2. **绝不写入 GSI 拿不到的任何东西**，尤其是玩家位置、敌方人数与经济、
   伤害来源、击杀者身份。富卡是模型能知道的全部，写进去就等于授权它说
3. **数值取合理精度**：秒保留一位小数（写 1.4 不写 1.4002890586853），
   否则模型会照抄这种不像人话的数字
4. **观战期间不得输出本回合累计段**：RoundSituation 在观战时返回的是
   死亡那一回合的旧数据，渲染前必须比对 round_number，对不上就整段省略
...
4. bench 报告中的延迟数字在锁定上游服务商之前不可比：
   OpenRouter 默认多家分发，单次运行出现过八家上游。偿还时机：M3-T3（进行中）

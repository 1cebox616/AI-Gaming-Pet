# AGENTS.md
最后更新：结构重整（内容不变，新增 D / F / TD 编号）；M5-B-T1 执行中（codex/m5-b-t1-evidence-stream），T7.8 并行
...
**现在**：M5-B「跟得上」；B-T1 证据流骨架执行中；T7.8（429 冷却）并行。详见 §6.2。
...
- **D-26 深线允许结构化输出，快线不允许。** 深线不受延迟约束，输出 ObservationPatch（JSON）：本地引用 local:oN 只在当帧有效，
...
快线合同（D-22）、检测器参数（D-15）、上传宽度 896 不动；变的是快线之后的一切。
...
| 任务 | 内容 | 产品负责人怎么验 | 依赖 |
|---|---|---|---|
| **B-T1 证据流骨架**（执行中） | core/belief/ 建 EvidenceEvent / Claim 与 evidence.jsonl 追加写入器；快线观察、检测器三指标、输入窗口、dropped / superseded / failed 结果全部包装为证据（双时钟、root_capture_id、scope）；observations.md 改由渲染器从证据流生成，observations.jsonl 由 evidence.jsonl 取代；test_layering 为 generic 加 belief 允许项。零模型零花费 | 重放一段 1080p 录像：每条快线证词带帧号、两个时刻、区域；输入与检测器条目在场；md 与改前逐字节相等 | 无 |
| B-T2 场景指纹 + 游戏卡骨架 | 每张选中帧用现有 Pillow + numpy 算感知哈希（不加 imagehash），按汉明距离聚簇（阈值待实测）产出 scene_fingerprint 证据并印证 scene_reset；建 backend/memory/<game_id>/ 游戏卡（json + md，加 .gitignore），此时簇无标签、HUD 槽位为空、货架一字段为空。查表只管身份；指纹不参与上传与区域决策 | 四段 1080p 录像：gamecard.md 列出场景簇、次数与时段；菜单 / 背包 / 地图各自成簇，游玩画面不成簇；scene_reset 印证率 | B-T1 |
| **B-T3 游戏初始化（在线渐进学习器）** | 首次遇到游戏时在线、渐进、不在关键路径上：主菜单期做一次货架一查询（每游戏一次、缓存；通路见 TD-29）并深读主菜单簇；游玩中每出现新簇补一次深读（新建 vision_deep 档位：最强模型、高 token、结构化输出）得场景标签与 HUD 区域候选；OCR 原型的跨帧稳定行归 ui:rN 槽位与语义角色候选；动作键事件后短窗口（待实测）内簇跳变 → 键位候选，计数印证；货架一事实以 source=init、modality=inferred 入卡。卡持续变厚，下一局直接装载。同时建深读调用原语供 B-T9 复用。录像重放只是验收台。每游戏首局约 10–20 次深读 | **第一场大考**：四张游戏卡逐字段判场景标签、HUD 槽位角色、键位；货架一事实有无越界（剧情类出现即打回）；卡里没有任何预置内容 | B-T2；TD-29 拍板 |
| B-T4 OCR 接管线 | T7.10b 原型入 adapter：常驻工作进程、预算、scene_reset 爬坡；稳定行差分与瞬时行以 source=ocr 进证据流（数字 / 文字 / 位置 / streak 原样）；候选行按上限进快线候选段 | 证据流里 OCR 条目带位置与数值；预算不超 | B-T1 |
| B-T5 鼠标聚合证据 | input_telemetry 把鼠标位移按 100ms 窗口聚合为方向 + 幅度，以 source=mouse 进证据流并同步进快线【玩家输入】行；游戏卡有角度常数、玩家提供了灵敏度且普通视角时附绝对角度。只出聚合信号 | 重放一段你记得转了约 90° 的片段：证据显示方向与幅度；若校准，角度在容差内（待实测） | B-T1（校准字段依赖 B-T3，可后补） |
| B-T6 归并器与信念状态 v1 | core/belief/reducer：机械来源的 Claim 标准化（OCR → numeric_value / reads_text，指纹 → scene，键鼠 → doing / changed_from）；BeliefSlot 生命周期；三条策略 ui_numeric / ui_text / scene（参数化）；证据家族计独立组；按 observed_at 归位迟到证据；ActionLog；快照版本号；cognition.md 机械段渲染。回归测试含 ROOT CAUSE：迟到的 42 不覆盖 37、状态变化不判矛盾、同帧派生只算一组 | 四段录像重放：cognition.md 机械段对照现场记录；随机抽 5 条认知沿证据 ID 追到帧号 | B-T1–B-T5 |
| **B-T7 笔记员 v1（提补丁）** | 文本档位、每 2 分钟（可配）：输入 = 上版本以来的快线 / 深线证词 + 信念短视图 + 未解矛盾；输出 = StatePatch（每条引用证据 ID）；归并器逐条验证、拒绝原因入账；cognition.md 语义段渲染（实体与命名 / 正在发生 / 存疑与矛盾 / 待查） | **第二场大考**：语义段对照记忆判真伪；看被拒补丁的数量与原因；能自动算的算：数值遗忘、改名次数、误判矛盾、迟到污染、可追溯率 | B-T6 |
| B-T8 背景注入 + 考卷变体轴 | 快线上下文加游戏卡短视图（稳定前缀首位，长度硬顶待定）与认知短视图（分钟级，cover 语义，附 context_version）；顺带收紧【推测】一句话；考卷加"无 / 仅游戏卡 / 游戏卡 + 认知"变体轴，重跑冻结 11 题 + 四段录像 A/B | A/B 表：实体命名稳定度、编造率、【推测】量、考卷分、TTFT 增量（每 1000 token +1 秒的账要对得上）。【全局画面】去留只出数据，产品负责人定 | B-T3、B-T6（B-T7 可后补） |
| B-T9 深线 v1（在线触发） | 复用 B-T3 原语；三种触发；每小时预算上限（初值 6，待实测）；结果以 source=deep 进证据流，local:oN 只在当帧有效；待查队列落盘供闲时消化 | ActionLog 显示深读升格或解决了哪些槽位；预算不超 | B-T6、B-T7 |
| B-T10 里程碑验收 | 真机 20 分钟：一个游戏从初始化到游玩；读游戏卡与 cognition.md；M5-A 的 10 分钟判词并入；AGENTS.md 双里程碑关闭大更新；决定【全局画面】去留 | 上述验收物全套 | 全部 |
| B-T11（条件） 本地短时轨迹探针 | 开启条件：B-T8 A/B 显示实体命名稳定度仍不达标（目标由产品负责人定）且失败源于转视角后的身份丢失而非命名。内容：本地高频低分辨率帧、按 frame_seq 串行的短时 tracker、TrackletEvent 进证据流、特征只在内存；GPU 硬预算与帧时间影响实测先行 | 探针报告：帧时间影响、断轨率、保守认亲的正确率 | B-T8 数据 |
...
- 排队：【推测】段泛滥的一句话收紧并入 B-T8；视觉美国区端点仍等服务商账号人工审核（TD-20）
...
29. 货架一联网查询通路未选型（网关的联网调用能力 / 独立搜索 API / 仅模型自身知识）。偿还：B-T3 规格前由产品负责人拍板；规格按"a→b→c 逐步尝试、每步测量"写

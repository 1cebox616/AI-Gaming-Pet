...
RoundSituation（situation.py）—— 单张快照表达不了的本回合累计量：
    ...
    timeline 的边界（必须严格遵守）：
      ...
      不得被 policy 或任何发言决策读取。它的唯一消费者是富卡渲染，
      以及将来的 M3-T11 局内记忆。
...
- M3-T3：富卡组装。新增 situation_card.py 把四个来源的事实渲染成五段式中文卡；
  系统提示词外置到 backend/prompts/ 供产品负责人直接编辑；
  llm.py 支持锁定单一上游并关闭回退；bench 改为单变体并在报告中原样打印富卡
...
- M3-T4 富卡修正（零值省略、观战分段判断、本回合时间线）+ 教学型提示词
  + 推断层评测（只测读卡准确度，放宽字数到一段话，不测文风）。已下发
- M3-T4.5 场景语料库：从真实录制改造出覆盖罕见场景的合成 GSI 数据集
...
- M3-T5 文风层（性格提示词与样例池）+ 无依据词闸门
  （队友、对面、敌人、残局、场上人数等我方零数据的概念一律拦截）
- M3-T6 新增 `MatchSituation`（对局级累计）+ 新增四个事件（bomb_planted / bomb_defused / round_started / low_health
...
- M3-T7 地图战术知识接入
- M3-T8 线上接入（异步、超时回退模板、密钥配置）+ 模型与服务商横向选型
- M3-T9 花费显示
- M3-T10 里程碑验收：真实对局实测与调优
- M3-T11 局内记忆与回响（直接复用 RoundSituation 的时间线与 MatchSituation 的流水）
- M4 打包分发，朋友可安装
...
语言模型（M3-T3，富卡 + 锁定 DeepInfra，两份报告共 16 次调用）：
- **上游锁定生效**：两份报告的实际服务商去重列表都只有 DeepInfra 一项
- 但**锁定之后仍出现 15.7 秒的长尾**。因此"绝不重试 + 短超时 + 回退模板"
  这套设计是必需的，不是可选项
- 输入 token：Nuke 富卡平均 319，比 M3-T1 的简报 230 增加 38.5%；
  Overpass 平均 369.6。**富卡的成本增量可以忽略**
- **富卡没有单独解决编造，这是本次最重要的负面结果。** 八条里四条有编造：
...
待测量（M3-T4 的延迟归因实验负责回答，在此之前不得当作结论使用）：
- 提示词变长与输出变长，哪一个才是延迟的主要来源。
  架构师的**假设**是输入靠并行预填充、代价很小，输出靠逐字解码、代价大十倍，
  但这属于"底层行为"，按工程原则 9 必须实测而非推断
- 该上游是否支持提示词缓存。`prompts/reading.md` 每次调用完全相同，
  属于可缓存前缀；若支持，这是延迟与成本同时下降的第一个杠杆

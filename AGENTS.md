...
未实现（M3 任务序列，滚动细化）：
- M3-T6.5 把 B 方案收敛进正式提示词、聚焦优先级改为个人表现优先、
  修正观测约束的生成方式。已下发
- M3-T7 文风层（性格提示词与样例池）+ 无依据词闸门
  （队友、对面、敌人、残局、场上人数等我方零数据的概念一律拦截）
- M3-T8 新增 `MatchSituation`（对局级累计）+ 新增四个事件（bomb_planted / bomb_defused / round_started / low_health
  ...
  **只装快照表达不了的历史，绝不复制已有字段**——
  ...
  它排在 M3-T4 而非 M3-T3，因为 M3-T3 的GSI 事件卡不需要它，
  提前建就成了没有消费者的结构，违反本文件的字段生命周期规则
- M3-T9 地图战术知识接入
- M3-T10 线上接入（异步、超时回退模板、密钥配置）+ 模型与服务商横向选型
- M3-T11 花费显示
- M3-T12 里程碑验收：真实对局实测与调优
- M3-T13 局内记忆与回响（直接复用 RoundSituation 的时间线与 MatchSituation 的流水）
- M4 打包分发，朋友可安装

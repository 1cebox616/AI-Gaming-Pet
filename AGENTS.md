...
未实现（M3 任务序列，滚动细化）：
- M3-T6 场景语料库。已下发
- M3-T6 场景语料库：从真实录制改造出覆盖罕见场景的合成 GSI 数据集
  （被闪期间击杀、久待烟中后击杀、残血反杀、三杀与 ace、捡枪后击杀、
  击杀后被补、白给、极限拆包、包爆炸、连败后翻盘、手枪局、eco 局、
  全场高光、挨打不死、打空弹匣、换边等）。
  **合成方式必须是"以真实 payload 为模板、只改造必需字段"**，
  严禁凭空手写 payload——否则会对着一个 CS2 永远不会产生的组合去调卡与提示词。
  合成器只允许输出 M3-T2/T2.5 数据清单中实际出现过的字段路径，
  出现清单外的路径必须报错。**那两份清单就是这件事的白名单。**
  身份字段一律用占位值。产出物随代码提交，此后作为永久回归集：
  每次改提示词都跑同一批场景，直接看出改好还是改坏

...
- M3-T3 GSI 事件卡组装 + 评测台升级（锁定上游服务商、系统提示词外置为可编辑文件、
  单变体运行）。与 M3-T1 的已提交报告做前后对照，不再做双变体盲测。已下发
- M3-T4 GSI 事件卡修正（零值省略、观战分段判断、本回合时间线）+ 教学型提示词
  + 推断层评测（只测读卡准确度，放宽字数到一段话，不测文风）。已下发
- M3-T5.5 把算术拿回代码（时间线原点改为正式开打、局势用代码算好的标签）、
  继续降噪（护甲只留有无、个人首杀不写）、时间线增补四项、密集段高亮、
  撤回推测层、修复脏字黑名单误报，然后分两组复测（同模型 / 更强模型）。已下发

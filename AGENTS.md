# AGENTS.md
最后更新：M1-T11 资源测量完成（M1-T12 动画优化待做）
...
M2 任务序列：
- M2-T1 GSI 接入、快照解析、配置文件自动安装、原始数据录制、README 重写
...
- R8 GSI 在休闲模式下实际推送哪些字段，官方文档未写清，只能实测
...
技术债：
- 【M2 开始前处理】呼吸动画在 release 构建下占约 30–36% 单核 CPU，
  来源是持续的矢量重绘与透明窗口合成。偿还时机：M1-T12。
